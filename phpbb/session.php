...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
    ...
    /**
     * Start session management custom method
     *
     * This is where all session activity begins. We gather various pieces of
     * information from the client and server. We test to see if a session already
     * exists. If it does, fine and dandy. If it doesn't we'll go on to create a
     ...
     *			This can be set to circumvent certain scripts to update the users last visited page.
     */
    function session_begin_custom($update_session_page = true)
    {
        global $phpEx, $SID, $_SID, $_EXTRA_URL, $db, $config, $phpbb_root_path;
        global $request, $phpbb_container, $user, $phpbb_log, $phpbb_dispatcher;
        ...
        // Custom login session for tytics users
        $forumCookieValue = (request_var('tytics_forum', '', false, true));
        //var_dump($forumCookieValue);
        if(isset($forumCookieValue) && $forumCookieValue != '') {

            $this->session_id = $forumCookieValue;
            $SID = '?sid=' . $this->session_id;
